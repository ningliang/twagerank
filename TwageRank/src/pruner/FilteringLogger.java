...
public class FilteringLogger implements ResultHandler {
	...
	public void handleResult(Result result) {
		...
		if (valid(result.getId())) {
			...
			if (result.isSuccessful()) {
				int[] idHolder = new int[result.getFollowerIds().length];
				for (int id : result.getFollowerIds()) if (valid(id)) idHolder[count++] = id;
				filteredIds = new int[count];
				for (int i = 0; i < count; i++) filteredIds[i] = idHolder[i];
			}
	...
	private Boolean valid(int id) {
		return (id < this.filterMap.length && this.filterMap[id] != 0);
	}
